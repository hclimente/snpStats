...
would be at least 0.5). This is carried out using the {\tt rect.hclust}
function, which returns a list giving the names of the members of each
cluster together with their positions in the rows and columns of the 
distance matrix\footnote{The command also superimposes rectangles
  corresponding to these clusters on an existing dendrogram plot}:
<<clusters>>=
clusters <- rect.hclust(hc, h=0.5)
clusters
@ 
It can be seen that there are \Sexpr{length(clusters)} clusters.
...
clusters. Of these, \Sexpr{sum(sapply(clusters, length)==1)} SNPs
