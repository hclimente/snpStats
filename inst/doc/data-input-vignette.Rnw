...
be read given the large memory capacity of modern computers. However
R imposes a limit of  $2^{31}-1 \sim 2\times 10^9$ elements in a single
array\footnote{This limit applies in both the 32-bit and 64-bit versions
  of R; these versions differ only in the {\em total} memory that can
  be used}. For example, this would correspond to one million loci for two
thousand subjects and would occupy two gigabytes of machine memory.
...
\end{enumerate}
Missing values in the last two fiels are usually coded as zero. 
...
{\tt data} sub-directory of the {\tt snpStats} package as, respectively,
...
<<sysfile>>=
pedfile <- system.file("data/sample.ped.gz", package="snpStats")
pedfile
infofile <- system.file("data/sample.info", package="snpStats")
@ 
...
read the files:
<<plink>>=
fam <- system.file("data/sample.fam", package="snpStats")
bim <- system.file("data/sample.bim", package="snpStats")
bed <- system.file("data/sample.bed", package="snpStats")
...
subset of data from a very large PLINK dataset. This is demonstrated
...
<<longfile>>=
longfile <- system.file("data/sample-long.gz", package="snpStats")
longfile
@ 
The first few lines of the file are given below
\begin{verbatim}
...
confidence measure for the call (here always {\tt 1.000}). We first
generate the subject and SNP identifiers 
<<ssid>>=
samples <- paste("subject", 1:100, sep="")
snps <- paste("snp", 1:18, sep="")
@ 
(more usually these would be read in from accompanying support
files). To read the long format file in and inspect the data:
<<readlong>>=
...
A few remarks:
\begin{enumerate}
\item We need to know the names of the subjects and SNPs before
  calling the function. But the input file could contain much more
  data than will be extracted by the function call.
\item Here, the order of subjects and SNPs in the file is the same as
  specified in the arguments {\tt sample.id} and {\tt
    snp.id}. Optionally this can be relaxed, although this will result
  in slower operation.
...
\item Here, calls were represented by a single genotype code. It is
  also possible to read calls as pairs of alleles.
\item the {\tt sep="$\backslash$t"} argument indicates a TAB-delimited file.
\end{enumerate}
