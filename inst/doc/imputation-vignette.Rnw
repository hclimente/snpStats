...
pdftitle={Imputed SNP analyses with snpAssoc}]
{hyperref}

\title{Imputed SNP analyses and meta-analysis with snpAssoc}
...
use of {\tt snpAssoc} for imputation, 
...
how {\tt snpAssoc} stores uncertain genotypes is provided by the
...
Note that the ability of {\tt snpAssoc} to store imputed genotypes in
...
<<mach,fig=TRUE>>=
mach <- read.mach(system.file("data/mach1.out.mlprob.gz", package="snpAssoc"))
...
The {\tt snpAssoc} package includes a number of tools to facilitate
this. All the tests implemented in {\tt snpAssoc} are ``score''
...
In {\tt snpAssoc}, the statistical testing functions can be
...
this, {\tt snpAssoc} provides the {\tt switch.alleles} function, which
