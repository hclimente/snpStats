...
ibsCount <- function(snps, uncertain=FALSE) {
  .Call("ibs_count", snps, uncertain, PACKAGE="snpAssoc")
}

ibsDist <- function(counts) {
  .Call("ibs_dist", counts, PACKAGE="snpAssoc")
}

snp.pre.multiply <- function(snps,  mat, frequency=NULL, uncertain=FALSE ) {
  .Call("snp_pre", snps, mat, frequency, uncertain, PACKAGE="snpAssoc")
}

snp.post.multiply <- function(snps,  mat, frequency=NULL, uncertain=FALSE) {
  .Call("snp_post", snps, mat, frequency, uncertain, PACKAGE="snpAssoc")
}

snp.cor <- function(x, y, uncertain=FALSE) {
  .Call("corsm", x, as.matrix(y), uncertain, PACKAGE="snpAssoc")
}
